...
{
    OutputDebugString(std::wstring(L"FastCgiClient::Connect, m_pSocket = " + std::to_wstring((uint64_t)m_pSocket.get()) +  L"\r\n").c_str());
    if (m_pSocket != nullptr)
    {
        //m_pSocket->BindFuncBytesReceived(static_cast<function<void(TcpSocket* const)>>(nullptr));
        //m_pSocket->BindErrorFunction(static_cast<function<void(BaseSocket* const)>>(nullptr));
    ...
    OutputDebugString(std::wstring(L"FastCgiClient::Connect, neue m_pSocket = " + std::to_wstring((uint64_t)m_pSocket.get()) + L"\r\n").c_str());
...
{
    OutputDebugString(std::wstring(L"FastCgiClient::SocketCloseing, m_pSocket = " + std::to_wstring((uint64_t)m_pSocket.get()) + L", pBaseSocket = " + std::to_wstring((uint64_t)pBaseSocket) + L"\r\n").c_str());
